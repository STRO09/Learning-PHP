...
$result = $conn->prepare("select id from user where uname=?");
$result->bind_param('s', $_SESSION["name"]);
$result->execute();
$result->store_result();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $blogimg = isset($_FILES["blog_img"]["name"]) ? $_FILES["blog_img"]["name"] : null;

    if (isset($_FILES["blog_img"]["name"])) {
        move_uploaded_file($_FILES["blog_img"]["tmp_name"], "uploads/blogs/" . $_FILES["blog_img"]["name"]);
    }

    $sql = $conn->prepare("insert into blog(title,content,img,user_id) values(?,?,?,?)");
    $sql->bind_param(
        'sssi',
        $title,
        $content,
    ...
    if ($sql->execute()) {
        echo "<script>alert('Blog was addedl!')</script>";
    ...
    <main>
        <div class="container p-5 ">
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="" class="form-label">Choose a picture to add:</label>
